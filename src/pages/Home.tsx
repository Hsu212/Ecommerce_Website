...
const Home: React.FC = () => {
  ...
  {slides.map((slide, index) => {
    ...
    let overlay = null;
    if (slide.id === 1) {
      overlay = { title: "New Arrivals", subtitle: "Fresh styles for modern living", cta: "Explore Now", link: "/products" };
    } else if (slide.id === 2) {
      overlay = { title: "Cozy Comfort", subtitle: "Relax in style with our latest collections", cta: "Discover More", link: "/products" };
    } else if (slide.id === 3) {
      overlay = { title: "Flash Sale", subtitle: "Up to 50% off on selected items", cta: "Shop Now", link: "/sales" };
    }
